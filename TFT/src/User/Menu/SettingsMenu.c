...
static char firmware_name[64]    = "Unknown system";  // firmware name
static char firmware_version[64] = "Unknown version"; // firmware version
char machine_type[64]            = "Unknown machine"; // machine type
...
void infoSetFirmwareVersion(char * version, uint8_t version_len)
{
  version_len++;  // add space for terminal '\0' character
  strncpy_no_pad(firmware_version, version, MIN(sizeof(firmware_version), version_len));
}
...
void menuInfo(void)
{
  ...
  const GUI_RECT version[7] = {
    {start_x, top_y + 0*BYTE_HEIGHT, LCD_WIDTH, top_y + 1*BYTE_HEIGHT},
    {start_x, top_y + 1*BYTE_HEIGHT, LCD_WIDTH, top_y + 3*BYTE_HEIGHT},
    {start_x, top_y + 3*BYTE_HEIGHT, LCD_WIDTH, top_y + 5*BYTE_HEIGHT},
  ...
  GUI_DispString(0, version[1].y0, (uint8_t *)"Version :");
  GUI_DispString(0, version[2].y0, (uint8_t *)"Machine :");
  GUI_DispString(0, version[3].y0, (uint8_t *)"Board   :");
  GUI_DispString(0, version[4].y0, (uint8_t *)"Firmware:");
  GUI_DispString(0, version[5].y0, (uint8_t *)"SPIFlash:");

  // draw info
  GUI_SetColor(0xDB40);
  GUI_DispStringInPrectEOL(&version[0], (uint8_t *)firmware_name);
  GUI_DispStringInPrectEOL(&version[1], (uint8_t *)firmware_version);
  GUI_DispStringInPrectEOL(&version[2], (uint8_t *)machine_type);
